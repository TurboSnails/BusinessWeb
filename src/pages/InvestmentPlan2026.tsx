...
const InvestmentPlan2026 = () => {
  ...
  const timelineData = [
    ...
    },
    {
      ...
      time: '盘后发布',
      priority: 'critical',
      completed: true, // 综合判断内容，不需要勾选框
      actions: [
        { id: 'jan12-1', text: '📊 数据纠正：BAC实际$13.1亿, -10%（笔记$16.0亿, +45%，严重偏差实际下降）、WFC实际$10.4亿, -5%（笔记$10.4亿, +42%，增速错误实际下降）、C实际$22.2亿, -14%（笔记$28.2亿, +31%，金额和增速都偏高），仅JPM $46.6亿, +77%准确（需标注Apple Card并表影响）' },
        { id: 'jan12-2', text: '🟡 当前阶段：信贷预警期（非确认做空期）- 仅JPM预警，其他三家平稳，信号强度被高估' },
        { id: 'jan12-3', text: '📈 信号灯状态：银行拨备🟡黄灯、Non-Accrual贷款🟢绿灯、SLOOS信贷标准🟢绿灯、HY利差2.74%🟢绿灯、XLF/SPY相对表现🟡黄灯' },
        { id: 'jan12-4', text: '💡 近期操作（1-2周）：降低仓位至70-75%，提高现金至15-20%，增配短久期国债5-10%+黄金5%，小仓位对冲3-5%（VIX看涨期权/SPY Put）' },
        { id: 'jan12-5', text: '❌ 禁止操作：大规模做空指数、抄底金融股、加杠杆抄底（时机未到）' },
        { id: 'jan12-6', text: '📊 关键监测指标：HY OAS利差（当前2.74%，黄灯>3.0%，红灯>3.5%）、Non-Accrual QoQ（Q1财报4月）、SLOOS净收紧（2月初）、消费者逾期率（1/20纽约联储）、XLF跑输SPY周数（每周五）' },
        { id: 'jan12-7', text: '🚨 发令枪：任意2个红灯同时亮起 → 考虑指数级做空' },
        { id: 'jan12-8', text: '🎯 中期策略（1-3个月）：情景A信号消退30%概率→2月中旬加仓至85%；情景B信号强化50%概率→分阶段减仓至30-50%；情景C系统性风险20%概率→紧急防御仓位≤20%' },
        { id: 'jan12-9', text: '🔍 下周关键事件：1/20纽约联储消费信贷报告（超级关键，若逾期率>2.8%立即减仓至70%）、1/15-17其他银行财报、1/22初请失业金数据' },
        { id: 'jan12-10', text: '⚠️ 必须避免：过早做空、忽视结构性因素、不设止损（若HY利差<2.5%且SLOOS宽松，承认预警失败）' }
      ],
      notes: '综合判断：当前处于"信贷预警期"而非"确认做空期"。框架正确但比市场早1-2个季度。正确做法：降低仓位+建立监测系统+等待确认信号，不是"现在做空"而是"为可能的做空做准备"。'
    },
    {
      date: '2026年1月17日',
      day: '周六',
